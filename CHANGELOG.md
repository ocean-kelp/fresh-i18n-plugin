...
- **AsyncLocalStorage error handling** - Added try-catch around `getStore()` calls
  - Prevents crashes when AsyncLocalStorage context is not active during SSR
  - Gracefully falls back to client-side `window.__I18N__` injection
  - Fixes "Cannot read properties of undefined" errors during island hydration

## [0.2.5] - 2026-01-27

### Fixed

- **Client bundling issue** - Removed `node:async_hooks` import from client code
  - Prevents Vite bundling errors in browser environments
  - Client now accesses AsyncLocalStorage singleton via global Symbol without importing Node.js modules
  - Critical fix for Vite-based Fresh applications
